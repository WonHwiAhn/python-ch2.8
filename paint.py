...
def test_to_string():
    p = Point()
    print(p)
    # print(p.__repr__())   # 요런 방식보다는 내장함수 사용하는게 좋다
    print(repr(p))
...
def main():
    print('# test_unbound_class_method()')
    test_unbound_class_method()
    print('# test_bound_instance_method()')
    test_bound_instance_method()
    print('# test_other_method()')
    test_other_method()
    print('# test_member()')
    test_member()
    ...
    test_to_string()
    print('# test_class_rect()')
    test_class_rect()
    print('# def test_oerator_overloading()')
